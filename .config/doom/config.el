...
;; Fallback dabbrev completion for prog-mode (cape), LSP still takes priority
; (use-package! cape
;   :config
...
(defun +opencode/vterm-open ()
  ...
  (interactive)
  (require 'vterm)
  (let ((buf (get-buffer-create "*opencode*"))
        (root (when (fboundp 'projectile-project-root)
                (projectile-project-root))))
    (when (and buffer-file-name (not (eq major-mode 'vterm-mode)))
      (auto-revert-mode 1))
